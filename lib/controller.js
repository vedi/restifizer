  ...
  bind() {
    ...
    _.forEach(_.sortBy(this.actions, 'priority'), (action) => {
      try {
        if (action.enabled) {
          ...
          action.method.forEach((method) => {
            action.transports.forEach(
              transport => transport.addRoute(this, method, this.path, action, scope => Bb
        ...
        if (this.log) {
          this.log.error(
            `Set route for action: ${action.name} and path ${this.path}/${action.path}`);
  ...
  unbind() {
    ...
    _.forEach(_.sortBy(this.actions, 'priority'), (action) => {
      try {
        ...
        if (this.log) {
          this.log.error(
            `Unset route for action: ${action.name} and path ${this.path}/${action.path}`);
          this.log.error('Error', err);
        } else {
// eslint-disable-next-line no-console
          console.log(`Unset route for action: ${action.name} and path ${this.path}/${action.path}`);
// eslint-disable-next-line no-console
          console.log('Error', err);
        }
