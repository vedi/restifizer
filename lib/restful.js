  ...
  insert: function (req, res, next) {
    ...
    Q
      ...
      .then(function (doc) {
        ...
        return Q.ninvoke(_this, 'createDocument', doc, req, res);
      })
      .then(function (doc) {
      ...
      .seq(function (collection) {
        ...
        try {
          if (_this.aggregateOptions.collectionPost) {
            _this.collectionPost.post(collection, req, res, this);
          } else {
  ...
  selectOne: function (req, res, next) {
    ...
    Seq()
      ...
      .seq(function (file, contentType) {
        ...
        try {
          if (_this.selectOneOptions.post) {
            _this.countOptions.post(file, req, res, function() {
...
function setResError(err, res, log) {
  ...
  if (!err) {
    err = new Error();
    err.httpStatus = HTTP_STATUSES.INTERNAL_ERROR;
  }
  else if (!(err instanceof Error)) {
    var newErr = new Error();
    // if HTTP_STATUSES
    if (err.code && err.message) {
      newErr.httpStatus = err;
    } else if (err.httpStatus) {
      newErr.httpStatus = err.httpStatus;
    }
  ...
  if (!err.httpStatus) {
    ...
    errorMessage = err.message || errorStatus.message;
  }
  else {
