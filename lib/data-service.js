...
class DataService {
  constructor(options) {
    ...
    requireOptions(options, ['dataSource']);
    const { dataSource, ...restOptions } = options;
    Object.assign(this, restOptions);

    this.dataSource = dataSource.type
      // eslint-disable-next-line global-require,import/no-dynamic-require
      ? require(`./data-sources/${dataSource.type}`)(dataSource.options)
      : dataSource;

    this.idField = this.idField || this.dataSource.defaultIdField;
    this.disablePagination = this.disablePagination || false;
  ...
  async selectOne(scope) {
    const model = await this.locateModel(
      scope,
      {
        strict: true,
        withQueryPipe: true,
  ...
  async delete(scope) {
    scope.model = await this.locateModel(
      scope,
      {
        strict: true,
        withQueryPipe: false,
  ...
  getFetchingFields(scope, fieldNames = scope.fields) {
    return fieldNames
      ? this._resolveFieldsMetadata(fieldNames)
      : _.clone(this.defaultFields, true);
  }
  ...
  async getFilter(scope) {
    ...
    if (_.isFunction(defaultFilter)) {
      defaultFilter = defaultFilter.call(this, scope);
    }
   ...
   * @param options.strict throws NOT_FOUND if no record found, default is `true`
   * @param options.withQueryPipe pass it through queryPipe, default is `true`
  ...
  async locateModel(
    ...
    const {
      fieldList = scope.fields,
      strict = true,
      withQueryPipe = true,
    } = options;
    await this._handlePre(scope);
    const filter = await this.buildConditions(scope);
    scope.fieldList = this.getFetchingFields(scope, fieldList);
  ...
  _resolveFieldsMetadata(fields) {
    const fieldsMetadata = _.pick(this.fieldMap, _.intersection(fields, this.modelFieldNames));
    if (!fieldsMetadata[this.idField]) {
      // we need to force adding id (only field with $slice, can lead to fetching all fields)
      fieldsMetadata[this.idField] = this.getField(this.idField);
    }
