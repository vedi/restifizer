...
class DataService {
  ...
  getFilter(scope) {
    return Bb
      ...
      .then((conditions) => {
        ...
        let queryFilter = filter || (body && body.filter);
  ...
  getPagination(scope) {
    ...
    if (!pagination.limit) {
      ...
      pagination.limit = pagination.limit <= this.config.maxPerPage ? pagination.limit : this.config.maxPerPage;
    }
   ...
   */
  assignFields(scope) {
    const fields = Object.keys(scope.source).filter((field) => {
  ...
  assignFilter(queryParams, fieldName, scope) {
    return _.includes(this.modelFieldNames, fieldName) &&   // It's an allowable field
  ...
  proceedArrayMethods(scope) {
    return Bb
      .map(this.arrayMethods, (methodName) => {
        ...
        if (methodBody) {
          const fields = Object.keys(methodBody).filter((field) => {
  ...
  buildConditions(scope) {
    const { params } = scope;
    return scope.source = _.pick(params, Object.keys(params));
  }
