...
class DataService {
  ...
  insert(scope) {
    return Bb
      ...
      .then((data) => {
        scope.source = _.assign(data, scope.req.body);
  ...
  getFilter(scope) {
    return Bb
      ...
      .then((conditions) => {
        ...
        return _.assign((queryFilter ? JSON.parse(queryFilter) : defaultFilter), conditions);
      });
  }
  ...
  createDocument(scope) {
    return Bb
      .resolve(this.dataSource.create(scope.source))
      .then((model) => {
        scope.model = model;
      });
  }
  ...
  _handlePre(scope) {
    if (_.isFunction(scope.action.pre)) {
      return scope.action.pre.call(this, scope);
    } else {
  ...
  _handlePost(model, scope) {
    if (_.isFunction(scope.action.post)) {
      return scope.action.post.call(this, scope);
    } else if (_.isFunction(this.post)) {
