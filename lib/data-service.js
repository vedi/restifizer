...
const setResData = utils.setResData;
...
class DataService {
  ...
  select(scope) {
    ...
    return Bb
      ...
      .then((filter) => {
        ...
        // q
        const q = scope.req.query.q;
        // orderBy
        const orderBy = this.getOrderBy(scope);
        // limit
        const limit = this.getLimit(this.config.defaultPerPage, this.config.maxPerPage, scope);
        // page
  ...
  insert(scope) {
    return Bb
      ...
      .then((data) => {
        scope.source = Object.assign(scope.source, data, scope.req.body);
  ...
  getFilter(scope) {
    return Bb
      ...
      .then((conditions) => {
        ...
        let queryFilter = scope.req.query.filter || scope.req.body.filter;
  ...
  getOrderBy(scope) {
    const orderBy = scope.req.query.orderBy;
    return orderBy ? JSON.parse(orderBy) : this.orderBy;
  }

  getLimit(defaultPerPage, maxPerPage, scope) {
    const perPage = scope.req.query.perPage || scope.req.query['per_page'];
    if (!perPage) {
  ...
  addLinkHeaders(page, limit, currentLength, scope) {
    const initialUrl = scope.req.url;
    ...
    const fullURL = scope.req.protocol + '://' + scope.req.get('host') + cleanedUrl;
    const links = {};
    // add prev
    if (page > 1) {
