...
class DataService {
  ...
  replace(scope) {
    return this
      .locateOne(scope, !this.smartPut, false)
      .then((model) => {
        if (!model) {
  ...
  createDocument(scope) {
    return Bb
      .resolve(this.dataSource.create(scope.dest))
      .then((model) => {
        scope.model = model;
      });
  }
