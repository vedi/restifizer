...
var RestifizerScope = require('./restifizer-scope');
...
function DataService(options) {
  ...
  if (this.defaultFields) {
    this.defaultFields = this._normalizeFields(this.defaultFields);
  } else {
    this.defaultFields = this.fieldMap;
  }

  // extract default field name list for quick access
...
_.extend(DataService.prototype, {
  ...
  extractFieldList: function (scope) {
    ...
    if (scope.req.query.fields) {
      ...
      fields = _.intersection(fields, this.defaultFieldNames);
      if (fields.length == 0) {
