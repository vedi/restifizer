...
_.extend(DataService.prototype, {
  ...
  select: function (scope) {
    ...
    return Promise
      ...
      .then(function(filter) {
        ...
        // limit
        var limit = this.getLimit(scope, this.config.defaultPerPage, this.config.maxPerPage);
        // page
      ...
      .then(function (model) {
        ...
        scope.model = model;
        if (this.collectionPost) {
          return this.collectionPost(scope);
        }
      })
      .then(function () {
        var _this = this;
        // run post processing
        if (this.post) {
          return Promise.map(scope.model, function (item) {
            return _this.post(item);
      ...
      .then(function () {
        this.addLinkHeaders(scope, page, limit);
        return scope.model;
      });
  },
  selectOne: function (scope) {
    return Promise
      ...
      .then(function () {
        return this.locateModel(scope, true);
      })
      .then(function (model) {
        scope.model = model;
        if (scope.model) {
  ...
  insert: function (scope) {
    return Promise
      ...
      .then(function () {
        if (this.beforeAssignFields) {
          return this.beforeAssignFields(scope);
        }
      })
      .then(function () {
        return this.assignFields(scope);
      })
      .then(function () {
        return this.createDocument(scope);
      })
      .then(function () {
        return this.beforeSave(scope);
      })
      .then(function () {
        return this.saveDocument(scope);
      })
      .then(function () {
        if (this.afterSave) {
          return this.afterSave(scope);
        }
      })
      .then(function () {
        if (this.afterChange) {
  ...
  replace: function (scope) {
    return Promise
      ...
      .then(function () {
        return this.locateModel(scope, false);
      })
      .then(function (model) {
        if (!model) {
          throw HTTP_STATUSES.NOT_FOUND.createError();
  ...
  getLimit: function (scope, defaultPerPage, maxPerPage) {
  ...
  addLinkHeaders: function (scope, page, limit) {
    var currentLength = scope.model.length;
   ...
   * @param scope
   */
  proceedArrayMethod: function proceedArrayMethod(queryParam, methodName, fieldName, scope) {
    return this.dataSource.proceedArrayMethod(queryParam, methodName, fieldName, scope);
  },
   ...
   * @param withQueryPipe
   */
  locateModel: function (scope, withQueryPipe) {
    return Promise
      .bind(this)
      .then(function() {
        return this.buildConditions(scope);
  ...
  buildConditions: function (scope) {
    ...
    return _.pick(params, _.keys(params));
  },
  ...
  setResError: function (scope, err) {
    setResError(scope, err, this.log, this.parseError, this.dataSource.parseError);
  },
            ...
            _.forEach(action.method, _.bind(function (method) {
              app[method](path + '/' + action.path,
                ...
                function (req, res, next) {
                  var scope = new RestifizerScope(action.handler, _this.contextFactory);
                  scope.req = req;
                  scope.res = res;
                  scope.dest = {};
                  scope.model = {};
                  if (scope.isInsert() || scope.isUpdate()) {
                    var paramKeys = _.keys(req.params);
                    var params = _.pick(req.params, paramKeys);
                    scope.source = _.assign(req.body, params);
                  }

                  _this[action.handler].call(_this, scope)
                    .then(function (data) {
                      if (typeof data != 'undefined') {
                        _this.setResData(scope, data);
                      }
                      next();
                    })
                    .catch(function (err) {
                      _this.setResError(scope, err);
                      next();
                    });
                },
                this.resultSender
