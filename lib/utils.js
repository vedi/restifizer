...
var setResData = function setResData(scope, data) {
  if (scope.req.method.toLowerCase() !== 'head') {
    scope.res.restfulResult = data;
  }
};

var setResError = function setResError(scope, err, log, controllerParseError, dsParseError) {
  var errorStatus,
    errorMessage,
    errorDetails;
  ...
  setResData(scope, {error: errorStatus.message, message: errorMessage, details: errorDetails});
  if (log) {
